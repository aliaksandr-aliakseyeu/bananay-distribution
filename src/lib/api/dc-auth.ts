...
export interface DcVerifyResponse {
  access_token: string;
...
export const dcAuthApi = {
  requestOtp: async (phone_e164: string): Promise<{ message?: string }> => {
    const res = await fetch(`${API_BASE_URL}/api/v1/auth/dc/request-otp`, {
      method: 'POST',
  ...
  verifyOtp: async (phone_e164: string, code: string): Promise<DcVerifyResponse> => {
    const res = await fetch(`${API_BASE_URL}/api/v1/auth/dc/verify-otp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  refreshToken: async (refresh_token: string): Promise<DcVerifyResponse> => {
