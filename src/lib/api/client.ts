...
async function refreshDriverToken(): Promise<string | null> {
...
export async function fetchDriverApi<T>(
  endpoint: string,
  options?: RequestInit
): Promise<T> {
  ...
  const isDriverRefreshRequest = endpoint.includes('/api/v1/auth/driver/refresh');
  if (res.status === 401 && !isDriverRefreshRequest) {
    const refreshedAccessToken = await refreshDriverToken();
  ...
  if (!res.ok) {
    ...
    throw new APIError(res.status, err.detail || String(res.status), err);
  }
...
export const apiClient = {
  get: <T>(endpoint: string, options?: RequestInit) =>
    fetchDriverApi<T>(endpoint, { ...options, method: 'GET' }),
  post: <T>(endpoint: string, data?: unknown, options?: RequestInit) =>
    fetchDriverApi<T>(endpoint, {
      ...options,
      method: 'POST',
  ...
  delete: <T>(endpoint: string, options?: RequestInit) =>
    fetchDriverApi<T>(endpoint, { ...options, method: 'DELETE' }),
};
